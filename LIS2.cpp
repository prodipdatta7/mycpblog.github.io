 ...
 *
 *  author  :   prodipdatta7
 *  created :   Thursday 07-May, 2020  04:14:28 PM
**/
...
struct LIS2 {
	vector < vector < int > > T, order ; 

	void build(std::vector< pii > v, int n){
		T.resize(n + 1) ; 
		order.resize(n + 1) ;
		sort(all(v),[](pii a, pii b){return a.ss < b.ss ;}) ;
		for(int i = 0 ; i < n ; ++i){
			for(int j = v[i].ff ; j <= n ; j += j & -j){
				T[j].push_back(0) ; 
				order[j].push_back(v[i].ss) ; 
			}
		}
		for(int i = 1 ; i <= n ; ++i){
			T[i].push_back(0) ; 
			order[i].push_back(inf) ; 
		}
	}
	...
	void add(int x, int y, int v){
		int n = sz(T[x]) ; 
		y = upper_bound(all(order[x]), y) - order[x].begin() ; 
	...
	int solve(std::vector< pii > &p, int n){
		build(p, n) ; 
		int res = 0 ;
		for(int i = 0 ; i < n ; ++i){
			int mx = ask(p[i].ff - 1, p[i].ss - 1) + 1 ; 
			res = max(res, mx) ; 
			update(p[i].ff, p[i].ss, mx, n) ; 
		}
		return res ; 
	}
} ;
...
int main() {
    ...
    while (test--) {
    	...
    	for(int i = 0 ; i < n ; ++i){
    		a[i].ff = upper_bound(all(v), a[i].ff) - v.begin() ; 
    	}
    	LIS2 obj ; 
    	cout << obj.solve(a, n) << '\n' ; 
    }
